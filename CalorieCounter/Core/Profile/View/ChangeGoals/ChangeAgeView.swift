...
struct ChangeAgeView: View {
    ...
    var onAgeChange: ((_ user: UserData) -> Void)?
        ...
        NormalButton(action: {
            updateWeightUser() {
                ...
                if let user = userGoal.user {
                    self.onAgeChange?(user)
                }
            }
        }, title: "Update", color: Color.cl_F24F1D(), width: Vconst.DESIGN_WIDTH_RATIO * 160)
    }
    
    private func updateWeightUser(completed: @escaping() -> Void) {
        userGoal.user?.age = value
